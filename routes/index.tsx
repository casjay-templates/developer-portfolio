...
const GITHUB_USER: string = Deno.env.get('GITHUB_USER')! || 'casjay';

export const handler: Handlers = {
  async GET(req, ctx) {
    const username = GITHUB_USER || 'casjay';
    try {
      if (!username) {
...
export default function Greet({ data, params }: PageProps) {
  ...
  const username = params.username || GITHUB_USER;
  return (
    <Layout title={error ? 'Error' : username}>
      <div className={tw`max-w-3xl mx-auto`}>
        <div className={tw``}>
          <img src={user.avatar_url} alt={user.name} className={tw`w-64 h-64 aspect-square rounded-full`} />
          ...
          <div className={tw`mt-4 flex items-center space-x-2`}>
            <a href={`/${username}/followers`} className={tw`flex items-center space-x-1 hover:underline`}>
              ...
              <p className={tw`text-gray-300 text-sm`}>{user.followers > 1 ? 'followers' : 'follower'}</p>
            </a>
            <span>&bull;</span>
            <a href={`/${username}/following`} className={tw`flex items-center space-x-1 hover:underline`}>
              <p className={tw`font-bold`}>{user.following}</p>
              <p className={tw`text-gray-300 text-sm`}>{user.following > 1 ? 'followings' : 'following'}</p>
            </a>
          </div>

          <div className={tw`mt-4 flex flex-col space-y-2`}>
            {user.company && <InfoItem text={user.company} icon={<Company />} />}
            {user.location && <InfoItem text={user.location} icon={<Location />} />}
            {user.blog && <InfoItem text={user.blog} icon={<Link />} />}
            {user.twitter_username && <InfoItem text={`@${user.twitter_username}`} icon={<Twitter />} />}
          </div>
        </div>
